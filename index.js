...
module.exports = async (client, m, msg) => {
  try {
    ...
    if (isCmd) {
      data = await fetchJson('https://api.countapi.xyz/hit/nielzie/visits')
      jumlahcmd = `${data.value}`
      dataa = await 
      fetchJson(`https://api.countapi.xyz/hit/nielzie/visits${moment.tz('Asia/Jakarta').format('DDMMYYYY')}/visits`)
    ...
    switch (command) {
      case 'runtime':
        ...
        break
      case 'menu':
        sendButMessage(from, `Hai ${pushname}\n- ${prefix}runtime\n- ${prefix}neko\n- ${prefix}waifu`,
      ...
      case "about":
        ...
        break
      case "neko": {
        nekod = await axios.get('https://waifu.pics/api/sfw/neko')
        client.sendMessage(from, { image: { url: nekod.data.url }, caption: 'Done!' }, { quoted: msg })
      }
        break
      case "waifu": {
